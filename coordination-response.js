(() => {
  ...
  function responseAttentionData(){
    ...
    if(role==="provider"){
      ...
      if(request.status==="proposal") return {state:"Waiting",on:false,title:"Waiting for "+ownerLabel+" review.",copy:"The provider response has been sent. The client must approve it or request changes.",href:"#client-response-review",link:"View sent response ↓"};
      if(request.status==="approved") return {state:"Action needed",on:true,title:"Start the approved "+config.shortTitle.toLowerCase()+" service.",copy:"The client approved the response. The provider can now begin the next service step.",href:workflowHref("provider"),link:"Continue workflow →"};
      if(request.status==="in-progress"){
        if(serviceKey==="title"&&!request.clientClosingConfirmed) return {state:"Waiting",on:false,title:"Waiting for client signing confirmation.",copy:"The title / settlement work is in progress, but the provider is waiting for the client to confirm the required closing or signing step.",href:workflowHref("provider"),link:"View workflow →"};
    ...
    if(request.status==="approved") return {state:"Waiting",on:false,title:"Waiting for the provider to begin.",copy:"The provider response has been approved. The participating company is responsible for the next step.",href:workflowHref(role),link:"View workflow →"};
    if(request.status==="in-progress"){
      if(serviceKey==="title"&&isOwner&&!request.clientClosingConfirmed) return {state:"Action needed",on:true,title:"Confirm the closing / signing step.",copy:"The settlement provider is waiting for your confirmation that the required signing step is complete.",href:workflowHref(role),link:"Open workflow →"};
